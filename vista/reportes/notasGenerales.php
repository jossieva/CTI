<?php
	...
	$iduser=$_SESSION['id'];
	$objeto= new modelo();
	$arrDatos=$objeto->getDataStudent($iduser);	
...
$html.="<table>";
	...
	if(($iduser!="")){						
		$sql = "SELECT c.NOMBRE_CURSO,c.COD_CURSO,p.COD_PERSONA,p.NOMBRE,p.APELLIDO, acg.COD_CURSO_GRADO FROM db_curso c JOIN db_asig_curso_grado acg ON c.COD_CURSO=acg.COD_CURSO JOIN db_grado g ON g.COD_GRADO=acg.COD_GRADO JOIN db_asig_grado_persona agp ON agp.COD_GRADO=g.COD_GRADO JOIN db_persona p ON p.COD_PERSONA=agp.COD_PERSONA WHERE agp.COD_PERSONA=".$iduser;
		try{				
	...
	if(!empty($arrDatos)){// verificamos que la consulta de cursos no vaya vacia.
		...
		foreach($arrDatos1 as $data1){							
			$html.= "<th style='color:#04369a'><span  style='transform: rotate(-90deg); text-align:center; font-size:12px'>".($data1['NOMBRE_UNIDAD'])."</span></th>";					
		}			
		...
		foreach ($arrDatos as $data){			
			...
			foreach($arrDatos1 as $data1){
				$sql3="SELECT a.NOMBRE_ACTIVIDAD, a.COD_UNIDAD, SUM(aag.NOTA) nota FROM db_curso cu JOIN db_asig_curso_grado acg ON acg.COD_CURSO=cu.COD_CURSO JOIN db_grado g ON g.COD_GRADO=acg.COD_GRADO JOIN db_plan_curso pc ON pc.COD_CURSO_GRADO=acg.COD_CURSO_GRADO JOIN db_actividad a ON a.COD_ACTIVIDAD=pc.COD_ACTIVIDAD JOIN db_asig_acti_grado aag ON aag.COD_ACTIVIDAD=a.COD_ACTIVIDAD JOIN db_asig_grado_persona gp ON gp.COD_ASIG_GRADO_P=aag.COD_ASIG_GRADO_P JOIN db_persona pe ON pe.COD_PERSONA=gp.COD_PERSONA JOIN db_unidad u ON u.COD_UNIDAD=a.COD_UNIDAD WHERE cu.COD_CURSO=".$data['COD_CURSO']."  AND pe.COD_PERSONA=".$iduser." AND u.COD_UNIDAD=".$data1['COD_UNIDAD'];							
				try{				
	...
	foreach($arrDatos1 as $data1){
		$sqlP = "SELECT g.NOMBRE_GRADO, a.COD_UNIDAD, SUM(aag.NOTA) nota FROM db_curso cu JOIN db_asig_curso_grado acg ON acg.COD_CURSO=cu.COD_CURSO JOIN db_grado g ON g.COD_GRADO=acg.COD_GRADO JOIN db_plan_curso pc ON pc.COD_CURSO_GRADO=acg.COD_CURSO_GRADO JOIN db_actividad a ON a.COD_ACTIVIDAD=pc.COD_ACTIVIDAD JOIN db_asig_acti_grado aag ON aag.COD_ACTIVIDAD=a.COD_ACTIVIDAD JOIN db_asig_grado_persona gp ON gp.COD_ASIG_GRADO_P=aag.COD_ASIG_GRADO_P JOIN db_persona pe ON pe.COD_PERSONA=gp.COD_PERSONA JOIN db_unidad u ON u.COD_UNIDAD=a.COD_UNIDAD WHERE pe.COD_PERSONA=".$iduser." AND u.COD_UNIDAD=".$data1['COD_UNIDAD']."  GROUP BY g.COD_GRADO";
		try{				
